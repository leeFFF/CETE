...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter()  # 记录运行文件

    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu)
    # 多gpu运行的时候用distribution, args.local_rank默认-1
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                tmp_eval_loss, logits, attn = outputs[:3]

                eval_loss += tmp_eval_loss.mean().item()
            nb_eval_steps += 1

            val_attention = attn[-1]
    ...
    return results, preds2


def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    if args.local_rank not in [-1, 0] and not evaluate:  # rank 就是GPU序号
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, preds = evaluate(args, model, tokenizer, prefix=global_step)
